...
export const DistrictBottomSheet: React.FC<DistrictBottomSheetProps> = ({
  ...
  // Compile all lightbox items with metadata from district photos and places
  const allLightboxItems: LightboxImageItem[] = useMemo(() => {
    if (!district) return [];
  ...
  const handleOpenPhoto = (url: string) => {
    const foundIndex = allLightboxItems.findIndex((item) => item.url === url);
            ...
            {district.photos && district.photos.length > 0 && (
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2.5 flex items-center gap-1.5">
                  ...
                  District Photo Album ({district.photos.length})
                </h3>
                <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                  {district.photos.map((photoUrl, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => handleOpenPhoto(photoUrl)}
                      className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden border border-white/[0.1] hover:border-emerald-400/80 flex-shrink-0 group cursor-pointer shadow-md transition-all hover:scale-105 active:scale-95"
                          ...
                          <div className="flex items-center gap-2 mt-2.5 overflow-x-auto pb-1 no-scrollbar">
                            {place.photos.map((photoUrl, pIdx) => (
                              <button
                                ...
                                type="button"
                                onClick={() => handleOpenPhoto(photoUrl)}
                                className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-white/[0.1] hover:border-emerald-400/80 flex-shrink-0 group cursor-pointer shadow-sm transition-all hover:scale-105 active:scale-95"
                                title="Click to view full photo"
                              >
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                  src={photoUrl}
                                  alt={`${place.name} Photo ${pIdx + 1}`}
                                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                                  loading="lazy"
                                />
                              </button>
